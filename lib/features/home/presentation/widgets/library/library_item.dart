...
class LibraryItem extends StatelessWidget {
        ...
        child: Row(
          children: [
            GestureDetector(
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    AppText(
                      text: file.name!.length >20? file.name!.substring(0,20): file.name!,
                      fontSize: 14.01,
            ...
            Padding(
              ...
              child: InkWell(
                onTap: () async{
                  // AppLogger.success('message${file.url!}');
                  await _launchURL(file.url!);
                },
                child: Container(
                  height: 44,
  ...
  Future<void> showFileDetails(BuildContext ctx) {
                        ...
                        children: [
                          ...
                          BlocBuilder<LibraryCubit,LibraryState>(
                            builder: (context,state){
                              final updatedFile = state.files.firstWhere(
                                    (element) => element.id == file.id,
                                // orElse: () => file, // Fallback to original file if not found
                              );
                              return Row(
                                children: [
                                  IconButton(onPressed: ()async{

                                    await ctx.read<LibraryCubit>().starFile(fileId: updatedFile.id!, isStar: updatedFile.isStared!);
                                  }, icon: Icon(Icons.star, color: updatedFile.isStared ??false  ?const Color(0xffFFC107) :Colors.grey.shade200 , size: 25,),padding: EdgeInsets.zero,),
                                  AppText(
                                    text: updatedFile.stars.toString(),
                                    fontSize: 12,
